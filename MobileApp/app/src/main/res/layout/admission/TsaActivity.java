...
public class TsaActivity extends AppCompatActivity {
    ...
    private TsaViewModel viewModel;
    private ViewGroup scoresContainer; // Dùng ViewGroup (LinearLayout) thay cho RecyclerView

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admission_tsa);

        viewModel = new ViewModelProvider(this).get(TsaViewModel.class);
        ...
        viewModel.loadTsaScores(schoolId);
    }
    ...
    private void setupToolbar() {
        ...
        ((TextView) findViewById(R.id.tvTitle)).setText("Phương thức xét tuyển điểm DG Tư Duy");
    }

    private void observeViewModel() {
        viewModel.getTsaScores().observe(this, tsaScores -> {
            if (tsaScores != null) {
                // Gọi hàm mới để điền dữ liệu vào LinearLayout
                populateScores(tsaScores);
            }
        });
    }
