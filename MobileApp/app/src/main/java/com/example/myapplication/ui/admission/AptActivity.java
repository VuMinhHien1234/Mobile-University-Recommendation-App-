...
public class AptActivity extends AppCompatActivity {
    ...
    private AptViewModel viewModel;
    private ViewGroup scoresContainer; // Dùng ViewGroup (LinearLayout) thay cho RecyclerView

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admission_apt);

        viewModel = new ViewModelProvider(this).get(AptViewModel.class);
        ...
        viewModel.loadAptScores(schoolId);
    }
    ...
    private void setupToolbar() {
        ...
        ((TextView) findViewById(R.id.tvTitle)).setText("Phương thức xét tuyển điểm DGNL HCM");
    }

    private void observeViewModel() {
        viewModel.getAptScores().observe(this, aptScores -> {
            if (aptScores != null) {
                // Gọi hàm mới để điền dữ liệu vào LinearLayout
                populateScores(aptScores);
            }
        });
    }
